...
namespace NodeFactory {
  ...
  auto createMeatPieces(glm::vec2 loc, glm::vec2 dir, int count) ->
    ...
    auto pieces = std::vector<std::tuple<std::shared_ptr<Node>,std::shared_ptr<b2BodyDef>,std::shared_ptr<b2FixtureDef>>>();
  ...
  auto _createMeatPiece() ->
    ...
    auto node = std::make_shared<Node>("meat_" + std::to_string(getId()));
  ...
  auto createBloodstain() ->
    ...
    auto node = std::make_shared<Node>("blood_" + std::to_string(getId()));
  ...
  auto createMainMenu() -> std::shared_ptr<Node> {
    ...
    auto node = std::make_shared<Node>("player1");
  ...
  auto createRandomEnemy(float difficulty, int random) -> std::shared_ptr<Node> {

    auto node = std::make_shared<Node>("enemy_" + std::to_string(getId()));
  ...
  auto createPlayer(int playerId, int lives, glm::vec2 loc) ->
    ...
    node->addBehavior<PlayerBehavior>(playerId, lives);
