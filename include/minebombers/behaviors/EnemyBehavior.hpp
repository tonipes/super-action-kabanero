...
public:
  EnemyBehavior(Node* node, float difficulty) : _difficulty(difficulty){
  ...
  virtual auto update(float delta, std::shared_ptr<Node> node) -> void override {

  }

protected:
  auto updateCommon(float delta, std::shared_ptr<Node> node) -> void {
